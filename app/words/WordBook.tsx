...
export default function WordBook() {
  ...
  return <main className="wordbook-page">
    ...
    <div className="wordbook-info"><strong>{results.length} từ</strong><span>Ưu tiên từ trong flashcard Cambridge Flyers bạn cung cấp; đây vẫn là bộ chọn lọc, chưa phải toàn bộ danh sách thi.</span></div>
    <div className="wordbook-grid">{results.map((item) => <article key={item.word} className={`word-tile level-${item.level}`}>
      <div className="word-emoji">{item.flyerSprite !== undefined
        ? <span className="word-sprite flyer-word-sprite" style={{ backgroundPosition: `${(item.flyerSprite % 5) * 25}% ${Math.floor(item.flyerSprite / 5) * (100 / 3)}%` }} />
        : item.sprite !== undefined
          ? <span className="word-sprite" style={{ backgroundPosition: `${(item.sprite % 5) * 25}% ${Math.floor(item.sprite / 5) * (100 / 3)}%` }} />
          : item.emoji}</div>
      <div><small>{item.topic} · {item.level}</small><h2>{item.word}</h2><p>{item.meaning}</p></div>
      <button onClick={() => say(item.word)} aria-label={`Nghe phát âm ${item.word}`}><Volume2 size={19} /></button>
    </article>)}</div>
